...
def initialize_dut(dut,config):
    ...
    cocotb_inputs  = 0
    cocotb_outputs = 0
    for i in range(MtcAutoPorts.n_input_interfaces(MtcAutoPorts)):
        cocotb_inputs = MtcAutoPorts.get_input_interface_ports(i) + cocotb_inputs

    for i in range(MtcAutoPorts.n_output_interfaces(MtcAutoPorts)):
...
def mtc_auto_test(dut):
    ...
    for n_ip_intf in range(MtcAutoPorts.n_input_interfaces(MtcAutoPorts)): # Add concept of interface
        for io in range(MtcAutoPorts.get_input_interface_ports(n_ip_intf)):
            driver = FifoDriver(
                dut.input_spybuffers[MtcAutoPorts.get_input_interface_ports(n_ip_intf)* n_ip_intf + io].spybuffer,
                dut.clock,
                "MtcAuto",
    ...
    for n_op_intf in range(MtcAutoPorts.n_output_interfaces(MtcAutoPorts)):
   ...
   ###Get Output Test Vector List for Ports across all output interfaces##
    output_tv_list        =  []
    single_interface_list = []
    for n_op_intf in range(MtcAutoPorts.n_output_interfaces(MtcAutoPorts)): # Add concept of interface
        single_interface_list = (events.parse_file_for_testvectors(
            filename=master_tv_file,
