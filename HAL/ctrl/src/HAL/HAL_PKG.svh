...
package HAL_CTRL;
   ...
   typedef struct HAL_CSM_CSM_LPGBT_UPLINK_UPLINK_MON_t {
      logic READY;
      logic FEC_ERR_CNT[16 - 1:0];
   };

   typedef HAL_CSM_CSM_LPGBT_UPLINK_UPLINK_MON_t HAL_CSM_CSM_LPGBT_UPLINK_UPLINK_MON_t_ARRAY[2 -1:0];

   typedef struct HAL_CSM_CSM_LPGBT_UPLINK_UPLINK_CTRL_t {
      logic RESET;
   };

   typedef HAL_CSM_CSM_LPGBT_UPLINK_UPLINK_CTRL_t HAL_CSM_CSM_LPGBT_UPLINK_UPLINK_CTRL_t_ARRAY[2 -1:0];
   ...
   typedef struct HAL_CSM_CSM_SC_MASTER_IC_MON_t {
      logic RX_DATA_FROM_GBTX[8 - 1:0];
      logic TX_READY;
      logic RX_DATA[32 - 1:0];
      logic RX_CHIP_ADR[7 - 1:0];
      logic RX_UP_PARITY_OK;
      logic RX_DOWN_PARITY_OK;
      logic RX_ERR;
      logic RX_VALID;
      logic RX_REG_ADR[16 - 1:0];
      logic RX_LENGTH[16 - 1:0];
   };

   typedef struct HAL_CSM_CSM_SC_MASTER_IC_CTRL_t {
      logic TX_START_WRITE;
      logic TX_START_READ;
      logic TX_GBTX_ADDR[8 - 1:0];
      logic TX_REGISTER_ADDR[16 - 1:0];
      logic TX_NUM_BYTES_TO_READ[16 - 1:0];
      logic TX_DATA_TO_GBTX[8 - 1:0];
      logic TX_WR;
   };

   typedef struct HAL_CSM_CSM_SC_MASTER_SCA_RX_RX_MON_t {
      logic RX_LEN[8 - 1:0];
      logic RX_ADDRESS[8 - 1:0];
      logic RX_CONTROL[8 - 1:0];
      logic RX_TRANSID[8 - 1:0];
      logic RX_ERR[8 - 1:0];
      logic RX_RECEIVED;
      logic RX_CHANNEL[8 - 1:0];
      logic RX_DATA[32 - 1:0];
   };

   typedef HAL_CSM_CSM_SC_MASTER_SCA_RX_RX_MON_t HAL_CSM_CSM_SC_MASTER_SCA_RX_RX_MON_t_ARRAY[3 -1:0];
   ...
   typedef struct HAL_CSM_CSM_SC_MASTER_CTRL_t {
      ...
      logic TX_CMD[8 - 1:0];
      logic TX_ADDRESS[8 - 1:0];
      logic TX_TRANSID[8 - 1:0];
      logic TX_CHANNEL[8 - 1:0];
      logic TX_DATA[32 - 1:0];
      logic SCA_ENABLE[3 - 1:0];
      logic START_RESET;
      logic START_CONNECT;
      logic START_COMMAND;
      logic INJ_CRC_ERR;
   ...
   typedef HAL_CSM_CSM_MON_t HAL_CSM_CSM_MON_t_ARRAY[36 -1:0];
   ...
   typedef HAL_CSM_CSM_CTRL_t HAL_CSM_CSM_CTRL_t_ARRAY[36 -1:0];
