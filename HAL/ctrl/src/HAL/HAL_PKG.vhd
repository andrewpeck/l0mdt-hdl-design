...
package HAL_CTRL is
  ...
  type HAL_CSM_CSM_SC_MASTER_IC_MON_t is record
    ...
    RX_DATA                    :std_logic_vector(31 downto 0);  -- Up to 4 bytes of GBT-IC read data
    RX_CHIP_ADR                :std_logic_vector( 6 downto 0);  -- LPGBT I2C chip address
    RX_UP_PARITY_OK            :std_logic;                      -- LPGBT uplink IC parity check was OK
    RX_DOWN_PARITY_OK          :std_logic;                      -- LPGBT downlink IC parity check was OK
    RX_ERR                     :std_logic;                      -- LPGBT IC decoder state machine error
    RX_VALID                   :std_logic;                      -- LPGBT IC decoder output data is valid
    RX_REG_ADR                 :std_logic_vector(15 downto 0);  -- LPGBT IC decoder register address 
    RX_LENGTH                  :std_logic_vector(15 downto 0);  -- LPGBT IC decoder received length of the transaction. You only can read the tail 4 bytes due to firmware limitations
  ...
  type HAL_CSM_CSM_SC_MASTER_SCA_RX_RX_MON_t_ARRAY is array(0 to 2) of HAL_CSM_CSM_SC_MASTER_SCA_RX_RX_MON_t;
  ...
  type HAL_CSM_CSM_MON_t_ARRAY is array(0 to 35) of HAL_CSM_CSM_MON_t;

  type HAL_CSM_CSM_CTRL_t is record
    LPGBT                      :HAL_CSM_CSM_LPGBT_CTRL_t;
    SC                         :HAL_CSM_CSM_SC_CTRL_t;   
  end record HAL_CSM_CSM_CTRL_t;
  type HAL_CSM_CSM_CTRL_t_ARRAY is array(0 to 35) of HAL_CSM_CSM_CTRL_t;
