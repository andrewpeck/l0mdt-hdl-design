...
entity MEM_INT_12A148D_map is
  generic (
    READ_TIMEOUT     : integer := 2048;
    ALLOCATED_MEMORY_RANGE : integer := 32
    );
  port (
...
end entity MEM_INT_12A148D_map;
architecture behavioral of MEM_INT_12A148D_map is
...
begin  -- architecture behavioral
  ...
  -------------------------------------------------------------------------------
  -------------------------------------------------------------------------------
  assert ((4*12) <= ALLOCATED_MEMORY_RANGE)
    report "MEM_INT_12A148D: Regmap addressing range " & integer'image(4*12) & " is outside of AXI mapped range " & integer'image(ALLOCATED_MEMORY_RANGE)
  severity ERROR;
  assert ((4*12) > ALLOCATED_MEMORY_RANGE)
    report "MEM_INT_12A148D: Regmap addressing range " & integer'image(4*12) & " is inside of AXI mapped range " & integer'image(ALLOCATED_MEMORY_RANGE)
  severity NOTE;
  ...
  begin  -- process reg_writes
    if reset_axi_n = '0' then                 -- asynchronous reset (active low)
      reg_data( 0)( 0)  <= DEFAULT_MEM_INT_12A148D_CTRL_t.SIGNALS.wr_req;
      reg_data( 0)( 1)  <= DEFAULT_MEM_INT_12A148D_CTRL_t.SIGNALS.wr_ack;
      reg_data( 0)( 2)  <= DEFAULT_MEM_INT_12A148D_CTRL_t.SIGNALS.rd_req;
      reg_data( 0)( 3)  <= DEFAULT_MEM_INT_12A148D_CTRL_t.SIGNALS.rd_ack;
      reg_data( 0)( 4)  <= DEFAULT_MEM_INT_12A148D_CTRL_t.SIGNALS.flush_req;
      reg_data( 0)( 5)  <= DEFAULT_MEM_INT_12A148D_CTRL_t.SIGNALS.freeze_req;
      reg_data( 0)( 8 downto  6)  <= DEFAULT_MEM_INT_12A148D_CTRL_t.SIGNALS.mem_sel;
