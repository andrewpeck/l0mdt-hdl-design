...
begin  -- architecture behavioral
  ...
  -------------------------------------------------------------------------------
  -------------------------------------------------------------------------------
  assert ((4*10241) <= ALLOCATED_MEMORY_RANGE)
    report "C2C_INTF: Regmap addressing range " & integer'image(4*10241) & " is outside of AXI mapped range " & integer'image(ALLOCATED_MEMORY_RANGE)
  severity ERROR;
  assert ((4*10241) > ALLOCATED_MEMORY_RANGE)
    report "C2C_INTF: Regmap addressing range " & integer'image(4*10241) & " is inside of AXI mapped range " & integer'image(ALLOCATED_MEMORY_RANGE)
  severity NOTE;
    ...
    if localRdReq = '1' then
      ...
      case to_integer(unsigned(localAddress(13 downto 0))) is
        ...
        when 1024 => --0x400
          ...
          localRdData(12)            <=  Mon.C2C(1).STATUS.PHY_LANE_UP;                     --Aurora phy lanes up
        ...
        when 5120 => --0x1400
          ...
          localRdData(12)            <=  Mon.C2C(2).STATUS.PHY_LANE_UP;                     --Aurora phy lanes up
