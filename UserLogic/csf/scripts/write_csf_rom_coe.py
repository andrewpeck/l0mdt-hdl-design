...
def main():
    parser = argparse.ArgumentParser(
        description='Write CSF firmware ROM coe files')
    ...
    parser.add_argument("-o",
                        ...
                        help="Output Dir",
                        default="../src/")
    parser.add_argument("--mbar_width",
                        type=int,
                        help="SL slope m width",
                        default=11)
    parser.add_argument("--mbar_multi",
                        type=float,
                        help="SL slope m multiplier",
                        default=1024.0)
    parser.add_argument("--inv_sqrt_m_width",
                        type=int,
                        help="1/sqrt(1+mbar^2) width",
                        default=18)
    parser.add_argument("--divider_width",
                        type=int,
                        help="divider width",
                        default=22)
    parser.add_argument("--mfit_width",
                        type=int,
                        help="mfit width",
                        default=15)
    ...
    for x in range(0, 2**args.mfit_width - 1):
        m = -2**(args.mfit_width - 1) + x + 0.5
        theta = 0
        theta |= int(
            floor(atan(args.mfit_mult / m) *
                  args.theta_glob_mult)) & int(2**args.theta_width - 1)
