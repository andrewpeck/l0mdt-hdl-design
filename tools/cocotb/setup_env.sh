...
#Add support to compile Verilog libraries
    ...
    sed -i '/\@echo \"vmap $(RTL_LIBRARY) $(SIM_BUILD)\/$(RTL_LIBRARY)" /a \\t@echo "vmap unisim  $(XILINX_SIM_LIB)/unisim " >> $@ ' $(find ./env -name Makefile.questa)  #Create all libraries in project, VHDL_LIB has all of them
