...
# created by tb create for test: mpl_mtc_auto
##################################################
...
import l0mdt_tb.testbench.mpl_mtc_auto.mpl_mtc_auto_wrapper as wrapper
from l0mdt_tb.testbench.mpl_mtc_auto.mpl_mtc_auto_ports import MplMtcPorts

# CREATORSOFTWAREBLOCKimport l0mdt_tb.testbench.mpl_mtc_auto.mpl_mtc_auto_block as mpl_mtc_auto_block
...
def mpl_mtc_auto_test(dut):
    ...
    ##
    mpl_mtc_auto_wrapper = wrapper.MplMtcWrapper(
        clock=dut.clock,
    ...
    for n_ip_intf in range(MplMtcPorts.n_input_interfaces): # Add concept of interface
        for io in range(MplMtcPorts.get_input_interface_ports(n_ip_intf)):
            ...
            mpl_mtc_auto_wrapper.add_input_driver(driver, n_ip_intf, io) #Add interface
    ...
    for n_op_intf in range(MplMtcPorts.n_output_interfaces):
        for io in range(MplMtcPorts.get_output_interface_ports(n_op_intf)): #Outputs):
            ...
            mpl_mtc_auto_wrapper.add_output_monitor(monitor, n_op_intf, io, active=active)
    mpl_mtc_auto_wrapper.sort_ports()
   ...
   ###Get Output Test Vector List for Ports across all output interfaces##
    ...
    send_finished_signal = mpl_mtc_auto_wrapper.send_input_events(
        input_tv_list,
    ...
    for n_op_intf in range(MplMtcPorts.n_output_interfaces):
        ...
        for n_oport, oport in enumerate(mpl_mtc_auto_wrapper.output_ports(n_op_intf)):
    ...
    for n_op_intf in range (MplMtcPorts.n_output_interfaces):
        events_are_equal,pass_count_i , fail_count_i, field_fail_count_i = events.compare_BitFields(tv_bcid_list, output_tvformats[n_op_intf],MplMtcPorts.get_output_interface_ports(n_op_intf) , num_events_to_process , recvd_events_intf[n_op_intf],tolerances=pl_mtc_tol);
