...
import l0mdt_tb.testbench.ult_tar.ult_tar_wrapper as wrapper
from l0mdt_tb.testbench.ult_tar.ult_tar_ports import UltTarPorts

# CREATORSOFTWAREBLOCKimport l0mdt_tb.testbench.ult_tar.ult_tar_block as ult_tar_block
...
def initialize_dut(dut,config):
    ...
    cocotb_inputs  = 0
    cocotb_outputs = 0
    for i in range(UltTarPorts.n_input_interfaces):
        cocotb_inputs = UltTarPorts.get_input_interface_ports(i) + cocotb_inputs

    for i in range(UltTarPorts.n_output_interfaces):
        cocotb_outputs = UltTarPorts.get_output_interface_ports(i) + cocotb_outputs


    input_fifos = [x.spybuffer for x in dut.input_spybuffers]

    cocotb_input_interfaces = UltTarPorts.n_input_interfaces;
...
def ult_tar_test(dut):
    ...
    inputs_station_id= [["" for x in range(UltTarPorts.get_input_interface_ports(y))]for y in range(UltTarPorts.n_input_interfaces)]
    inputs_thread_n= [[0 for x in range(UltTarPorts.get_input_interface_ports(y))]for y in range(UltTarPorts.n_input_interfaces)]
    outputs_station_id= [["" for x in range(UltTarPorts.get_output_interface_ports(y))]for y in range(UltTarPorts.n_output_interfaces)]
    tolerance= [["" for x in range(UltTarPorts.get_output_interface_ports(y))]for y in range(UltTarPorts.n_output_interfaces)]
    outputs_thread_n= [[0 for x in range(UltTarPorts.get_output_interface_ports(y))]for y in range(UltTarPorts.n_output_interfaces)]
    for i in range(UltTarPorts.n_input_interfaces):
    ...
    # CREATORSOFTWAREBLOCK##
    # CREATORSOFTWAREBLOCKult_tar_block_instance = ult_tar_block.ult_tarBlock(dut.clock, "ult_tarBlock")
    # CREATORSOFTWAREBLOCKfor i, io in enumerate(UltTarPorts.Inputs):
    # CREATORSOFTWAREBLOCK    ult_tar_block_instance.add_fifo(
    # CREATORSOFTWAREBLOCK        dut.input_spybuffers[i].spybuffer,
    # CREATORSOFTWAREBLOCK        dut.clock,
    # CREATORSOFTWAREBLOCK        f"{ult_tar_block_instance.name}_Input_{i}",
    # CREATORSOFTWAREBLOCK        io,
    # CREATORSOFTWAREBLOCK        direction="in",
    # CREATORSOFTWAREBLOCK    )
    # CREATORSOFTWAREBLOCKfor i, io in enumerate(UltTarPorts.Outputs):
    # CREATORSOFTWAREBLOCK    ult_tar_block_instance.add_fifo(
    # CREATORSOFTWAREBLOCK        dut.output_spybuffers[i].spybuffer,
    # CREATORSOFTWAREBLOCK        dut.clock,
    # CREATORSOFTWAREBLOCK        f"{ult_tar_block_instance.name}_Output_{i}",
    # CREATORSOFTWAREBLOCK        io,
    # CREATORSOFTWAREBLOCK        direction="out",
    # CREATORSOFTWAREBLOCK    )
    # CREATORSOFTWAREBLOCKult_tar_block_instance.start()
    ...
    ##
    ## initialize the UltTar block wrapper
    ##
    ult_tar_wrapper = wrapper.UltTarWrapper(
        clock=dut.clock,
        name=f"UltTarWrapper",
    )
    ...
    for n_ip_intf in range(UltTarPorts.n_input_interfaces): # Add concept of interface
        for io in range(UltTarPorts.get_input_interface_ports(n_ip_intf)):
            driver = FifoDriver(
            ...
            ult_tar_wrapper.add_input_driver(driver, n_ip_intf, io) #Add interface

    sb_oport_index = 0
    for n_op_intf in range(UltTarPorts.n_output_interfaces):
        for io in range(UltTarPorts.get_output_interface_ports(n_op_intf)): #Outputs):
            active = True
            monitor = FifoMonitor(
            ...
            ult_tar_wrapper.add_output_monitor(monitor, n_op_intf, io, active=active)
    ult_tar_wrapper.sort_ports()
    ...
    for n_ip_intf in range(UltTarPorts.n_input_interfaces): # Add concept of interface
        single_interface_list = (events.parse_tvlist(
            ...
            n_ports = UltTarPorts.get_input_interface_ports(n_ip_intf),
            n_to_load=num_events_to_process,
   ...
   ###Get Output Test Vector List for Ports across all output interfaces##
    ...
    recvd_events_intf = []
    for n_op_intf in range(UltTarPorts.n_output_interfaces):
        recvd_events     = [["" for x in range(num_events_to_process)]for y in range(UltTarPorts.get_output_interface_ports(n_op_intf))]
        recvd_time     = [["" for x in range(num_events_to_process)]for y in range(UltTarPorts.get_output_interface_ports(n_op_intf))]
        for n_oport, oport in enumerate(ult_tar_wrapper.output_ports(n_op_intf)):
    ...
    events.results_summary(
        ...
        pass_count,
        fail_count,
        UltTarPorts.n_output_interfaces,
        field_fail_cnt_header,
        field_fail_cnt,
        total_ports=UltTarPorts.n_output_ports(UltTarPorts)
    )
