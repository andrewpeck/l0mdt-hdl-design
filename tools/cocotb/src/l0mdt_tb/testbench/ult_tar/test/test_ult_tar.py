...
def ult_tar_test(dut):
    ...
    test_config.read_io_config(config['testvectors'],UltTarPorts)
    ...
    for n_ip_intf in range(UltTarPorts.n_input_interfaces): # Add concept of interface
        single_interface_list = (events.parse_tvlist(
            ...
            n_to_load=num_events_to_process,
            station_ID=UltTarPorts.config_inputs['station_id'][n_ip_intf],
            tv_type=input_tvtype[n_ip_intf],
            cnd_thrd_id = UltTarPorts.config_inputs['thread_n'][n_ip_intf],
            tv_df_type = "MDT",
   ...
   #    cocotb.log.debug("\n\n\n\n\nGet Output Test Vector List for Ports across all output interfaces")
    ...
    for n_op_intf in range (UltTarPorts.n_output_interfaces):
        events_are_equal, pass_count_i , fail_count_i, field_fail_count_i  = events.compare_BitFields_new(
            ...
            UltTarPorts.config_outputs['tolerance'][n_op_intf],
            output_dir,
            stationNum=events.station_list_name_to_id(UltTarPorts.config_outputs['station_id'][n_op_intf]),
    ...
    events.results_summary(
        ...
        field_fail_cnt_header,
        field_fail_cnt,
        total_ports=UltTarPorts.n_output_ports()
    )
