...
import l0mdt_tb.testbench.polmux_tar.polmux_tar_wrapper as wrapper
from l0mdt_tb.testbench.polmux_tar.polmux_tar_ports import PolmuxTarPorts

# CREATORSOFTWAREBLOCKimport l0mdt_tb.testbench.polmux_tar.polmux_tar_block as polmux_tar_block
...
def initialize_dut(dut,config):
    ...
    cocotb_inputs  = 0
    cocotb_outputs = 0
    for i in range(PolmuxTarPorts.n_input_interfaces):
        cocotb_inputs = PolmuxTarPorts.get_input_interface_ports(i) + cocotb_inputs

    for i in range(PolmuxTarPorts.n_output_interfaces):
        cocotb_outputs = PolmuxTarPorts.get_output_interface_ports(i) + cocotb_outputs


    input_fifos = [x.spybuffer for x in dut.input_spybuffers]

    cocotb_input_interfaces = PolmuxTarPorts.n_input_interfaces;
...
def polmux_tar_test(dut):
    ...
    inputs_tv_df_type= [["" for x in range(PolmuxTarPorts.get_input_interface_ports(y))]for y in range(PolmuxTarPorts.n_input_interfaces)]
    outputs_tv_df_type= [["" for x in range(PolmuxTarPorts.get_output_interface_ports(y))]for y in range(PolmuxTarPorts.n_output_interfaces)]
    inputs_station_id= [["" for x in range(PolmuxTarPorts.get_input_interface_ports(y))]for y in range(PolmuxTarPorts.n_input_interfaces)]
    inputs_thread_n= [[0 for x in range(PolmuxTarPorts.get_input_interface_ports(y))]for y in range(PolmuxTarPorts.n_input_interfaces)]
    outputs_station_id= [["" for x in range(PolmuxTarPorts.get_output_interface_ports(y))]for y in range(PolmuxTarPorts.n_output_interfaces)]
    tolerance= [["" for x in range(PolmuxTarPorts.get_output_interface_ports(y))]for y in range(PolmuxTarPorts.n_output_interfaces)]
    outputs_thread_n= [[0 for x in range(PolmuxTarPorts.get_output_interface_ports(y))]for y in range(PolmuxTarPorts.n_output_interfaces)]
    for i in range(PolmuxTarPorts.n_input_interfaces):
        if "station_ID" in testvector_config_inputs[i] :
            ...
            if inputs_station_id[i]=="mezz_idx_to_station":
                ...
                if len(inputs_station_id[i])!=PolmuxTarPorts.get_input_interface_ports(i):
                    cocotb.log.fatal(f"len(inputs_station_id[i]) {len(inputs_station_id[i])} different from PolmuxTarPorts.get_input_interface_ports(i) {PolmuxTarPorts.get_input_interface_ports(i)}")
                    raise Exception
    ...
    # CREATORSOFTWAREBLOCK##
    # CREATORSOFTWAREBLOCKpolmux_tar_block_instance = polmux_tar_block.polmux_tarBlock(dut.clock, "polmux_tarBlock")
    # CREATORSOFTWAREBLOCKfor i, io in enumerate(PolmuxTarPorts.Inputs):
    # CREATORSOFTWAREBLOCK    polmux_tar_block_instance.add_fifo(
    # CREATORSOFTWAREBLOCK        dut.input_spybuffers[i].spybuffer,
    # CREATORSOFTWAREBLOCK        dut.clock,
    # CREATORSOFTWAREBLOCK        f"{polmux_tar_block_instance.name}_Input_{i}",
    # CREATORSOFTWAREBLOCK        io,
    # CREATORSOFTWAREBLOCK        direction="in",
    # CREATORSOFTWAREBLOCK    )
    # CREATORSOFTWAREBLOCKfor i, io in enumerate(PolmuxTarPorts.Outputs):
    # CREATORSOFTWAREBLOCK    polmux_tar_block_instance.add_fifo(
    # CREATORSOFTWAREBLOCK        dut.output_spybuffers[i].spybuffer,
    # CREATORSOFTWAREBLOCK        dut.clock,
    # CREATORSOFTWAREBLOCK        f"{polmux_tar_block_instance.name}_Output_{i}",
    # CREATORSOFTWAREBLOCK        io,
    # CREATORSOFTWAREBLOCK        direction="out",
    # CREATORSOFTWAREBLOCK    )
    # CREATORSOFTWAREBLOCKpolmux_tar_block_instance.start()
    ...
    ##
    ## initialize the PolmuxTar block wrapper
    ##
    polmux_tar_wrapper = wrapper.PolmuxTarWrapper(
        clock=dut.clock,
        name=f"PolmuxTarWrapper",
    )
    ...
    sb_iport_index = 0
    for n_ip_intf in range(PolmuxTarPorts.n_input_interfaces): # Add concept of interface
        for io in range(PolmuxTarPorts.get_input_interface_ports(n_ip_intf)):
            ...
            polmux_tar_wrapper.add_input_driver(driver, n_ip_intf, io) #Add interface

    sb_oport_index = 0
    for n_op_intf in range(PolmuxTarPorts.n_output_interfaces):
        for io in range(PolmuxTarPorts.get_output_interface_ports(n_op_intf)): #Outputs):
            active = True
            monitor = FifoMonitor(
            ...
            polmux_tar_wrapper.add_output_monitor(monitor, n_op_intf, io, active=active)
    polmux_tar_wrapper.sort_ports()
    ...
    for n_ip_intf in range(PolmuxTarPorts.n_input_interfaces): # Add concept of interface
        single_interface_list = (events.parse_tvlist(
            ...
            n_ports = PolmuxTarPorts.get_input_interface_ports(n_ip_intf),
            n_to_load=num_events_to_process,
    ...
    send_finished_signal = polmux_tar_wrapper.send_input_events(
        input_tv_list,
    ...
    recvd_valid = False
    for n_op_intf in range(PolmuxTarPorts.n_output_interfaces):
        recvd_events     = [["" for x in range(num_events_to_process)]for y in range(PolmuxTarPorts.get_output_interface_ports(n_op_intf))]
        recvd_time     = [["" for x in range(num_events_to_process)]for y in range(PolmuxTarPorts.get_output_interface_ports(n_op_intf))]
        for n_oport, oport in enumerate(polmux_tar_wrapper.output_ports(n_op_intf)):
    ...
    for n_op_intf in range (PolmuxTarPorts.n_output_interfaces):
        events_are_equal, pass_count_i , fail_count_i, field_fail_count_i  = events.compare_BitFields_new(
            tv_bcid_list, 
            output_tvformats[n_op_intf],
            PolmuxTarPorts.get_output_interface_ports(n_op_intf) , 
    ...
    events.results_summary(
        ...
        pass_count,
        fail_count,
        PolmuxTarPorts.n_output_interfaces,
        field_fail_cnt_header,
        field_fail_cnt,
        total_ports=PolmuxTarPorts.n_output_ports(PolmuxTarPorts)
    )
