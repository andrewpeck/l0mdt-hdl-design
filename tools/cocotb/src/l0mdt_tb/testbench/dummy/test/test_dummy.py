...
import l0mdt_tb.testbench.dummy.dummy_wrapper as wrapper
from l0mdt_tb.testbench.dummy.dummy_ports import DummyPorts

# CREATORSOFTWAREBLOCKimport l0mdt_tb.testbench.dummy.dummy_block as dummy_block
...
def initialize_dut(dut,config):
    ...
    cocotb_inputs  = 0
    cocotb_outputs = 0
    for i in range(DummyPorts.n_input_interfaces):
        cocotb_inputs = DummyPorts.get_input_interface_ports(i) + cocotb_inputs

    for i in range(DummyPorts.n_output_interfaces):
        cocotb_outputs = DummyPorts.get_output_interface_ports(i) + cocotb_outputs


    input_fifos = [x.spybuffer for x in dut.input_spybuffers]

    cocotb_input_interfaces = DummyPorts.n_input_interfaces;
...
@cocotb.test()
def dummy_test(dut):
    cocotb.log.info("This is DUMMY cocotb test - nothing has been implemented on the cocotb side")
...
#     inputs_station_id= [["" for x in range(DummyPorts.get_input_interface_ports(y))]for y in range(DummyPorts.n_input_interfaces)]
#     inputs_thread_n= [[0 for x in range(DummyPorts.get_input_interface_ports(y))]for y in range(DummyPorts.n_input_interfaces)]
#     outputs_station_id= [["" for x in range(DummyPorts.get_output_interface_ports(y))]for y in range(DummyPorts.n_output_interfaces)]
#     tolerance= [["" for x in range(DummyPorts.get_output_interface_ports(y))]for y in range(DummyPorts.n_output_interfaces)]
#     outputs_thread_n= [[0 for x in range(DummyPorts.get_output_interface_ports(y))]for y in range(DummyPorts.n_output_interfaces)]
#     for i in range(DummyPorts.n_input_interfaces):
...
#     # CREATORSOFTWAREBLOCK##
#     # CREATORSOFTWAREBLOCKdummy_block_instance = dummy_block.dummyBlock(dut.clock, "dummyBlock")
#     # CREATORSOFTWAREBLOCKfor i, io in enumerate(DummyPorts.Inputs):
#     # CREATORSOFTWAREBLOCK    dummy_block_instance.add_fifo(
#     # CREATORSOFTWAREBLOCK        dut.input_spybuffers[i].spybuffer,
#     # CREATORSOFTWAREBLOCK        dut.clock,
#     # CREATORSOFTWAREBLOCK        f"{dummy_block_instance.name}_Input_{i}",
#     # CREATORSOFTWAREBLOCK        io,
#     # CREATORSOFTWAREBLOCK        direction="in",
#     # CREATORSOFTWAREBLOCK    )
#     # CREATORSOFTWAREBLOCKfor i, io in enumerate(DummyPorts.Outputs):
#     # CREATORSOFTWAREBLOCK    dummy_block_instance.add_fifo(
#     # CREATORSOFTWAREBLOCK        dut.output_spybuffers[i].spybuffer,
#     # CREATORSOFTWAREBLOCK        dut.clock,
#     # CREATORSOFTWAREBLOCK        f"{dummy_block_instance.name}_Output_{i}",
#     # CREATORSOFTWAREBLOCK        io,
#     # CREATORSOFTWAREBLOCK        direction="out",
#     # CREATORSOFTWAREBLOCK    )
#     # CREATORSOFTWAREBLOCKdummy_block_instance.start()
...
#     ##
#     ## initialize the Dummy block wrapper
#     ##
#     dummy_wrapper = wrapper.DummyWrapper(
#         clock=dut.clock,
#         name=f"DummyWrapper",
#     )
...
#     sb_iport_index = 0
#     for n_ip_intf in range(DummyPorts.n_input_interfaces): # Add concept of interface
#         for io in range(DummyPorts.get_input_interface_ports(n_ip_intf)):
#             driver = FifoDriver(
...
#             dummy_wrapper.add_input_driver(driver, n_ip_intf, io) #Add interface

#     sb_oport_index = 0
#     for n_op_intf in range(DummyPorts.n_output_interfaces):
#         for io in range(DummyPorts.get_output_interface_ports(n_op_intf)): #Outputs):
#             active = True
#             monitor = FifoMonitor(
...
#             dummy_wrapper.add_output_monitor(monitor, n_op_intf, io, active=active)
#     dummy_wrapper.sort_ports()
...
#             n_ports = DummyPorts.get_input_interface_ports(n_ip_intf),
#             n_to_load=num_events_to_process,
#             station_ID=inputs_station_id[n_ip_intf],
#             tv_type=input_tvtype[n_ip_intf],
#             cnd_thrd_id = inputs_thread_n[n_ip_intf]
#             ))
#         for io in range(DummyPorts.get_input_interface_ports(n_ip_intf)): #Outputs):
#             input_tv_list.append(single_interface_list[io])

#    ###Get Output Test Vector List for Ports across all output interfaces##
#     output_tv_list        =  []
#     single_interface_list = []
#     for n_op_intf in range(DummyPorts.n_output_interfaces): # Add concept of interface
#         single_interface_list = (events.parse_tvlist(
#             tv_bcid_list,
#             tvformat=output_tvformats[n_op_intf],
#             n_ports = DummyPorts.get_output_interface_ports(n_op_intf),
#             n_to_load=num_events_to_process,
...
#     send_finished_signal = dummy_wrapper.send_input_events(
#         input_tv_list,
...
#     recvd_events_intf = []
#     for n_op_intf in range(DummyPorts.n_output_interfaces):
#         recvd_events     = [["" for x in range(num_events_to_process)]for y in range(DummyPorts.get_output_interface_ports(n_op_intf))]
#         recvd_time     = [["" for x in range(num_events_to_process)]for y in range(DummyPorts.get_output_interface_ports(n_op_intf))]
#         for n_oport, oport in enumerate(dummy_wrapper.output_ports(n_op_intf)):
...
#             tv_bcid_list, 
#             output_tvformats[n_op_intf],
#             DummyPorts.get_output_interface_ports(n_op_intf) , 
...
#         pass_count,
#         fail_count,
#         DummyPorts.n_output_interfaces,
#         field_fail_cnt_header,
#         field_fail_cnt,
#         total_ports=DummyPorts.n_output_ports(DummyPorts)
#     )
